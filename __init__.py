...
def ocr_and_fuzzy_check(
    tesseract_path: str,
    allpics: Union[list, tuple],
    strings_to_compare: Union[list, tuple, np.ndarray],
    compare_single_words: bool = True,
    compare_grouped_words: bool = True,
    scorer_single_words: valid_scorer = "WRatio",
    scorer_grouped_words: valid_scorer = "WRatio",
    add_after_tesseract_path: str = "",
    add_at_the_end: str = "-l eng --psm 3",
    **kwargs,
) -> pd.DataFrame:
    r"""
    Perform OCR on a list of images using Tesseract and perform fuzzy string matching
    ...
    b = np.array(strings_to_compare, dtype="U")
    comparelist = []

    if compare_single_words:
        comparelist.append(["ocr_result", "word", getattr(fuzz, scorer_single_words)])
    if compare_grouped_words:
        comparelist.append(
            ["grouped_text", "group", getattr(fuzz, scorer_grouped_words)]
        )

    for l in comparelist:
        a = df[l[0]].__array__().astype("U")
        ...
        df[rf"compared_{l[1]}_similarity"] = max_values
        df[rf"compared_{l[1]}_index"] = max_indices
        df[rf"compared_{l[1]}_value"] = b[df[f"compared_{l[1]}_index"]]
